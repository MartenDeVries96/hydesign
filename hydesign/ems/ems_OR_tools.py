...
def ems_cplex(
    ...
    for ib, batch in enumerate(batches):
        ...
        E_SOC_ts_batch, penalty_batch = ems_ortools_parts(
            wind_ts = wind_ts_sel,
...
def ems_ortools_parts(
    ...
    """EMS solver implemented in ortools
    ...
    # Define decision varibales
    P_HPP_t = [solver.NumVar(0, hpp_grid_connection, f'P_HPP_{t}') 
                     for t in range(N_t)]
    
    P_charge_t = [solver.NumVar(0, P_batt_MW, f'P_charge_{t}') 
                     for t in range(N_t)]
    P_discharge_t = [solver.NumVar(0, P_batt_MW, f'P_discharge_{t}') 
                     for t in range(N_t)]

    z_t = [solver.BoolVar( f'P_charge_discharge_{t}') 
                     for t in range(N_t)]                 
     
    P_curtailment_t = [solver.NumVar(0, wind_ts[t] + solar_ts[t],
                                  f'P_curtailment_{t}') 
                                  for t in range(N_t)]
    E_SOC_t = [solver.NumVar(0, np.max(E_batt_MWh_t),
                                  f'E_SOC_{t}') 
                                  for t in range(N_t+1)]
    penalty = solver.NumVar(-solver.infinity(), solver.infinity(), 'penalty')
    e_penalty = solver.NumVar(-solver.infinity(), solver.infinity(), 'e_penalty')

    e_penalty_aux = solver.NumVar(-solver.infinity(), solver.infinity(), 'e_penalty_aux')

    delta_P_charge_discharge_aux_t = [solver.NumVar(-2*P_batt_MW, 2*P_batt_MW, f'delta_P_charge_discharge_{t}') 
                     for t in range(N_t-1)] 

    # Define constraints
    solver.Add(e_penalty == ( e_peak_period_expected - solver.Sum(P_HPP_t[i] for i in peak_hours_index)) )
    # "only positive"
    solver.Add( e_penalty_aux >= 0 )
    solver.Add( e_penalty_aux >= e_penalty )
    solver.Add( penalty == price_peak*e_penalty_aux )
       
    # Intitial and end SOC
    solver.Add( E_SOC_t[0] == 0.5 * E_batt_MWh_t[0] )
    
    # SOC at the end of the year has to be equal to SOC at the beginning of the year
    solver.Add( E_SOC_t[-1] == 0.5 * E_batt_MWh_t[0] )
    ...
    for t in range(N_t):
        ...
        # representation of charge vs dischrage effciency 
        #solver.Add( P_charge_discharge_aux_t[t]  >= P_charge_discharge_t[t]/charge_efficiency )
        #solver.Add( P_charge_discharge_aux_t[t]  >= P_charge_discharge_t[t]*charge_efficiency )
        solver.Add( E_SOC_t[t+1] == E_SOC_t[t] - P_discharge_t[t]/charge_efficiency * dt + P_charge_t[t]*charge_efficiency * dt)

        solver.Add( P_discharge_t[t]<=z_t[t]*P_batt_MW)
        solver.Add( P_charge_t[t]<=(1-z_t[t])*P_batt_MW)
        ...
        solver.Add(
            ...
            solar_ts[t] +
            - P_curtailment_t[t] +
            P_discharge_t[t] - P_charge_t[t])
        
        solver.Add(E_SOC_t[t] <= E_batt_MWh_t[t])
        solver.Add(E_SOC_t[t] >= (1 - battery_depth_of_discharge)*E_batt_MWh_t[t])
        
        if t<N_t-1:
           solver.Add(delta_P_charge_discharge_aux_t[t] >= P_discharge_t[t+1] - P_charge_t[t+1] - (P_discharge_t[t] - P_charge_t[t]))
           solver.Add(delta_P_charge_discharge_aux_t[t] >= P_discharge_t[t] - P_charge_t[t] - (P_discharge_t[t+1] - P_charge_t[t+1]))

    # Objective function: maximize profit
    profit = solver.Sum(P_HPP_t[t] * price_ts[t] for t in range(N_t)) - penalty - solver.Sum(
                                  delta_P_charge_discharge_aux_t[t]*cost_of_battery_P_fluct_in_peak_price_ratio*price_ts_to_max[t]
                                  for t in range(N_t-1)
                        )  
    ...
    if status == pywraplp.Solver.OPTIMAL:
        P_charge_ts = [P_charge_t[t].solution_value() for t in range(N_t)]
        P_discharge_ts = [P_discharge_t[t].solution_value() for t in range(N_t)]
        P_charge_discharge_ts = [item1 - item2 for item1, item2 in zip(P_discharge_ts, P_charge_ts)]
        ...
        output = pd.concat([pd.DataFrame(P_charge_discharge_ts, columns = ['P_b']), 
                            pd.DataFrame(P_HPP_ts, columns = ['P_hpp']),
                            pd.DataFrame(P_curtailment_ts, columns = ['P_cur'])], axis = 1)
...
def ems_cvxpy_parts(
    ...
    """EMS solver implemented in cvxpy
    ...
    # Define decision varibales
    P_HPP_t = cp.Variable(N_t)
    
    P_charge_discharge_t = cp.Variable(N_t)

    P_charge_discharge_aux_t = cp.Variable(N_t)                
     
    P_curtailment_t = cp.Variable(N_t)
    E_SOC_t = cp.Variable(N_t+1)


    penalty = cp.Variable()
    e_penalty = cp.Variable()

    e_penalty_aux = cp.Variable()

    delta_P_charge_discharge_aux_t = cp.Variable(N_t-1) 

    # Define constraints
    constraints = []

    # add bounds
    constraints.append(P_HPP_t <= hpp_grid_connection)
    constraints.append(P_HPP_t >= 0)
    constraints.append(P_charge_discharge_t <= P_batt_MW)
    constraints.append(P_charge_discharge_t >= -P_batt_MW)
    constraints.append(P_charge_discharge_aux_t <= P_batt_MW)
    constraints.append(P_charge_discharge_aux_t >= -P_batt_MW)
    constraints.append(E_SOC_t <= np.max(E_batt_MWh_t))
    constraints.append(E_SOC_t >= 0)
    constraints.append(delta_P_charge_discharge_aux_t <= 2*P_batt_MW)
    constraints.append(delta_P_charge_discharge_aux_t >= -2*P_batt_MW)

    constraints.append(e_penalty == ( e_peak_period_expected - cp.sum(P_HPP_t)) )
    # "only positive"
    constraints.append( e_penalty_aux >= 0 )
    constraints.append( e_penalty_aux >= e_penalty )
    constraints.append( penalty == price_peak*e_penalty_aux )
       
    # Intitial and end SOC
    constraints.append( E_SOC_t[0] == 0.5 * E_batt_MWh_t[0] )
    
    # SOC at the end of the year has to be equal to SOC at the beginning of the year
    constraints.append( E_SOC_t[-1] == 0.5 * E_batt_MWh_t[0] )
    ...
    for t in range(N_t):
        # add bounds
        constraints.append(P_curtailment_t[t]>=0)
        constraints.append(P_curtailment_t[t]<=wind_ts[t] + solar_ts[t])
        # representation of charge vs dischrage effciency 
        constraints.append( P_charge_discharge_aux_t[t]  >= P_charge_discharge_t[t]/charge_efficiency )
        constraints.append( P_charge_discharge_aux_t[t]  >= P_charge_discharge_t[t]*charge_efficiency )
        constraints.append( E_SOC_t[t+1] == E_SOC_t[t] - P_charge_discharge_aux_t[t] * dt)

        constraints.append(
            P_HPP_t[t] == wind_ts[t] +
            solar_ts[t] +
            - P_curtailment_t[t] +
            P_charge_discharge_t[t])
        
        constraints.append(E_SOC_t[t] <= E_batt_MWh_t[t])
        constraints.append(E_SOC_t[t] >= (1 - battery_depth_of_discharge)*E_batt_MWh_t[t])
        
        if t<N_t-1:
           constraints.append(delta_P_charge_discharge_aux_t[t] >= P_charge_discharge_t[t + 1] - P_charge_discharge_t[t])
           constraints.append(delta_P_charge_discharge_aux_t[t] >= P_charge_discharge_t[t] - P_charge_discharge_t[t + 1])

    # Objective function: maximize profit
    profit = cp.sum(cp.multiply(P_HPP_t, price_ts)) - penalty 
    profit -= cp.sum(cp.multiply(delta_P_charge_discharge_aux_t, cost_of_battery_P_fluct_in_peak_price_ratio*price_ts_to_max[:N_t-1]))  
    ...
    if problem.status == cp.OPTIMAL:
        P_charge_discharge_ts = P_charge_discharge_t.value
        E_SOC_ts = E_SOC_t.value
        P_curtailment_ts = P_curtailment_t.value
        P_HPP_ts = P_HPP_t.value

        penalty_2 = penalty.value
        penalty_ts = np.ones(N_t) * (penalty_2/N_t)
        ...
        print("Solve status is:", problem.status)
